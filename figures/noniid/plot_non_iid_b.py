...
def plot_panel(ax, curves_dict, subtitle=None, show_legend=False, legend_loc="best"):
    ...
    # x 轴固定 0–100；主刻度 0、20、40、60、80、100
    ...
    # y 轴固定 0–100，并只标 0、40、60、80、100
    ax.set_ylim(0, 100)
...
# 新增：按固定 b 聚合不同 j 的曲线
def build_series_moderate_for_b(b):
    series = {}
    if b not in MODERATE_PATHS:
        return series
    for j in sorted(MODERATE_PATHS[b].keys()):
        path = MODERATE_PATHS[b][j]
        if path:
            series[f"j={j}"] = load_series(path)
    return series


def build_series_extreme_for_b(b):
    series = {}
    if b not in EXTREME_PATHS:
        return series
    for j in sorted(EXTREME_PATHS[b].keys()):
        path = EXTREME_PATHS[b][j]
        if path:
            series[f"j={j}"] = load_series(path)
    return series

def main():
    # 固定列为不同 b；每个子图里画不同 j 的曲线
    if not MODERATE_PATHS:
        raise RuntimeError("MODERATE_PATHS is empty; nothing to plot.")
    bs = sorted(MODERATE_PATHS.keys())
    if not bs:
        raise RuntimeError("No b values found in MODERATE_PATHS.")

    fig, axes = plt.subplots(2, len(bs), figsize=(22, 9.8), dpi=SAVE_DPI, sharex=False, sharey=True)

    # 上排：Moderate；下排：Extreme。每列固定一个 b
    for col, b in enumerate(bs):
        ax_m = axes[0, col]
        plot_panel(
            ax_m,
            build_series_moderate_for_b(b),
            subtitle=f"b={b}",
            show_legend=False,
        )
        ...
        plot_panel(
            ax_e,
            build_series_extreme_for_b(b),
            subtitle=f"b={b}",
            show_legend=SHOW_LEGEND and col == len(bs) - 1,
            legend_loc="lower right",
        )

    # 只保留一条全局 X/Y label（共享 y，仅一条）
